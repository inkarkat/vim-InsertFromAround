...
function! InsertFromAround#RecordColumn()
    let s:previousCol = virtcol('.')
    let s:isAtEndOfLine = (col('.') == col('$'))
    return ''
endfunction
function! InsertFromAround#IndentNewLine()
...
function! InsertFromAround#InsertToggleFromAround( isAbove, isToggle )
    if ! exists('b:lastInsertFromAroundInsertLine') || b:lastInsertFromAroundInsertLine != line('.')
...
function! InsertFromAround#InsertIndentFromAround()
